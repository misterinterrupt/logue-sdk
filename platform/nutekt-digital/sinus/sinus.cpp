               ...
               const uint32_t numFrames)
{
  ...
  q31_t * currentFrame = (q31_t *)firstFrame;
  const q31_t * lastFrame = currentFrame + numFrames;
  
  for (; currentFrame != lastFrame; ) {
    ...
    // const float w0 = s.w00 = osc_w0f_for_note((params->pitch)>>8, params->pitch & 0xFF);
    // float p = phi0 + osc_sinf(phi0);
    // p = (p <= 0) ? 1.f - p : p - (uint32_t)p;
    // float sig = osc_wave_scanf(s.wave0, phi0);

    float sig  = osc_sinf(phi0);
